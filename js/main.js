window.onload = function() {
    ...
    chrome.runtime.onMessage.addListener(function(request, sender, sendResponse){
        if(request.type == "get") {
            ...
            cList = [].concat(request.body);
            chrome.storage.local.set({wikidotCandyBox: cList});
            sendResponse(cList);
        }
    });
    
    chrome.storage.local.get(["wikidotCandyBox"], function(items) {
        if(chrome.extension.lastError !== undefined || !items || !items.wikidotCandyBox) {
            var base = [];
        }else {
            var base = items.wikidotCandyBox;
        }
        (function _f(cb) {
            ...
            WIKIDOT.page.callbacks.editClick = function(b){
                ...
                if(b.status == "ok") {
                    (function _f() {
                        ...
                        cList = t.value.split(/\s|[^a-zA-Z]/).filter(v=>v&&v.length>4);
